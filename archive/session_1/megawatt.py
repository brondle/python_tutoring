...
for i in range(len(compass)):
    ...
    for j in range(8):
        ...
        print('j/2: ', int(j/2))
        print('int', int(3.7))
        # this will always return <= 3, since int() in python rounds down
        para = para + ['', ' again', ' once again', ' yet again'][int(j/2)]
    #interesting aesthetic choice here - using + instead of full string to fit within lines
    print('para: ', para)
    print('para - 14: ', para[:-14])
    # because we know that the final phrase will end with 'and then yet again'
    para = (para[:-14] + 'so on, over and over again, many many times, ' +
            'until he reached his destination, and could sit down.')
    text.append(para)
    # shift first element in compass array into last position
...
## why is he using '4' instead of len(doors)? or len(doors) instead of '4'? try adding a fifth door here and see what happens
