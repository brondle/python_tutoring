...
def combine(num, words): #num is the iteration number in the loop calling the funciton (i+1)
    final = [] #a list called final is created within the function, this variable is returned when the funciton is completed
    if num > 0 and len(words) >= num:#is there an instance where this is not met? it is when the list of words is empty.
        print ('num: ', num)
        if num == 1:#num will be 1 on its last iteration, correct?this is the base case. all of the recursion ends here
            final = final + [[words[0]]] #the whole list plus the first item in the words list, why is this in double brackets?
            print ('final in if statement', final)
        else:#the double array/list places in a subarray, it concatenates it to a list
            final = final + [[words[0]] +
                    c for c in combine(num - 1, words[1:])] #can you explain this list comprehension? I get that it is calling the funciton
            # for c in combine(num - 1, words[1:]):  
            #     print(‘c: ‘, c) 
            #this is the non-list comprehension way of doing it.
            print('final in else statement', final) #the line above builds a single array each time it is run, in other other words the internal array builds one and the one below builds the whole.
        final = final + combine(num, words[1:])#this one builds the array of arrays. in the program flow, can you explain the role of this final (in relation to the others?)
    return final
