...
for i, items in enumerate(tags):
    if items[0] == my_phrase:
        new_group = (items[0], tags[i+1][0], tags[i+1][1])
        my_my_list.append(new_group)
print('my phrases ', my_my_list )
for i, items in enumerate(my_my_list):
    if items[2] == 'NN':
        my_my_string = items[0] + " "+ items[1]
        my_my_list_refined.append(my_my_string)

for i in range(len(my_my_list_refined)):
    if my_my_list_refined[i] not in my_no_repeat:
        my_no_repeat.append(my_my_list_refined[i])

print('my my noun ', len(my_my_list_refined), my_my_list_refined)
print('my my noun no repeat ', len(my_no_repeat), my_no_repeat)
cleaned_noun_phrases = []
print(type(noun_phrases))
for i in range(len(noun_phrases)):
    print('the item is ', noun_phrases[i])
    if noun_phrases[i].isalpha():
        print('post is alpha '), noun_phrases[i]
        if noun_phrases[i] not in cleaned_noun_phrases:
            cleaned_noun_phrases.append(noun_phrases[i])
print('the cleaned is alpha noun phrases are ', cleaned_noun_phrases)
